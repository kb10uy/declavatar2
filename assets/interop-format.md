...
- The FFI stores one blob per compile inside the caller's context and exposes it as a pointer and a length that stay valid until the next compile; [c-ffi.md](c-ffi.md) has the rules. The magic makes the blob self-identifying, so one compile entry point returns either kind.
