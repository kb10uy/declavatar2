...
For each encoded Rust enum, a test-only macro takes one entry per variant: an explicit variant pattern paired with a fixture expression. From this single list it generates both an exhaustive `match` without a wildcard fallback and the runnable fixture cases. Each case checks that its fixture matches its paired pattern and is encoded and decoded by the format tests. Adding a variant without a corresponding fixture then fails to compile. Do not use catch-all or alternative patterns that allow one fixture to stand for multiple variants. Nested enums are covered separately, including enums flattened into a single wire discriminator. This checks variant coverage, not every possible field value; boundary and malformed-input cases remain separate tests.
...
- Before implementing `AnimatorParameter` encoding, change the model and transform to retain typed defaults (`bool`, `i32`, `f32`). The current `Option<f32>` storage loses integer precision before encoding; converting it back to an integer in the encoder cannot recover the original value.
